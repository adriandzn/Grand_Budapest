...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['btn_save_reservation'])) {
    ...
    $status = $conn->real_escape_string($_POST['reservation_status']);
...
// ==========================================
// 3. BACKEND RESERVATION DELETION LOGIC (NEW)
// ==========================================
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['btn_delete_reservation'])) {
    ...
    $stmt = $conn->prepare("DELETE FROM tbl_reservationdetails WHERE reservation_id = ?");
...
<div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-4 gap-3">
    ...
    <div class="d-flex gap-2 w-100 mobile-w-auto justify-content-md-end" style="max-width: 600px;">
        <form method="POST" action="" class="d-flex gap-2 flex-grow-1">
            <input type="search" name="searchinput" value="<?php echo isset($_POST['searchinput']) ? htmlspecialchars($_POST['searchinput']) : ''; ?>" placeholder="Search guests..." class="form-control rounded-pill border-secondary shadow-sm">
            ...
            <table class="table table-hover align-middle">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="fw-bold"><?php echo htmlspecialchars($res['full_name']); ?><br><small class="text-muted"><?php echo htmlspecialchars($res['email']); ?></small></td>
                        ...
                        <td class="text-center">
                            <div class="d-flex gap-2 justify-content-center align-items-center">
                                <button type="button" class="btn btn-sm btn-outline-primary rounded-pill px-3" data-bs-toggle="modal" data-bs-target="#editResModal_<?php echo $res['reservation_id']; ?>">
                                    Edit
                                </button>
                                ...
                                <form method="POST" action="" onsubmit="return confirm('Are you completely sure you want to permanently delete Reservation #<?php echo $res['reservation_id']; ?> for <?php echo htmlspecialchars($res['full_name'], ENT_QUOTES); ?>?');" class="m-0">
                                    <input type="hidden" name="reservation_id" value="<?php echo $res['reservation_id']; ?>">
                                    <button type="submit" name="btn_delete_reservation" class="btn btn-sm btn-danger rounded-pill px-3">
                                        Delete
                                    </button>
                                </form>
                            </div>
                            ...
                            <div class="modal-content rounded-4 border-0 shadow-lg">
                                ...
                                <form method="POST" action="">
                                    <div class="modal-body p-4">
                                        ...
                                        <div class="row g-3 mb-3">
                                            <div class="col-md-6">
                                                ...
                                                <input type="text" name="full_name" value="<?php echo htmlspecialchars($res['full_name']); ?>" class="form-control rounded-3" required>
                                            </div>
                                            <div class="col-md-6">
                                                <label class="form-label fw-semibold">Contact Number</label>
                                                <input type="text" name="contact" value="<?php echo htmlspecialchars($res['contact']); ?>" class="form-control rounded-3" required>
                                            </div>
                                        </div>
                                        <div class="mb-3">
                                            <label class="form-label fw-semibold">Email Address</label>
                                            <input type="email" name="email" value="<?php echo htmlspecialchars($res['email']); ?>" class="form-control rounded-3" required>
                                        </div>
                                        <div class="row g-3 mb-3">
                                            <div class="col-md-6">
                                    ...
                                    <div class="modal-footer bg-light border-0 py-3 rounded-bottom-4">
                                        ...
                                        <button type="submit" name="btn_update_reservation" class="btn btn-primary fw-bold rounded-pill px-4 shadow-sm">Save Changes</button>
                                    </div>
                                </form>
                            </div>
                        </div>
