<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Grand Budapest Hotel</title>
    <link rel="stylesheet" href="css/bootstrap.min.css">
    <link rel="stylesheet" href="css/body.css">
</head>
<body>
    
    <!-- NAVBAR -->
    <?php include 'navbar.php'; ?>
    
    <!-- Hero Banner -->
    <img src="images/index-hero.png" alt="hero banner" class="img-fluid w-100 shadow">
    

    <!-- FIRST SECTION: Call to Action -->
    <section class="bg-lightbrown">
        <div class="container px-4">
            <div class="row py-5">

                <!-- Image on the left -->
                <div class="col-md-6 col-12 text-center d-flex flex-column justify-content-center p-5"><img src="images/hotel_pictures/lobby-art.png" alt="" class="img-fluid w-100 rounded shadow"></div>

                <!-- Text on the right -->
                <div class="col-md-6 col-12 d-flex flex-column justify-content-center font-white p-5">
                    <div class="row">
                        <div class="col font-title h2 fw-bold">5 STAR VINTAGE LUXURY EXPERIENCE</div>
                    </div>
                    <div class="row pt-2">
                        <div class="col font-body">The Grand Budapest Hotel is a timeless icon of luxury. Experience world-class service at the most prestigious hotel in Zubrowka. From elegant rooms to exceptional amenities, every moment offers a truly unforgettable stay.</div>
                    </div>

                    <!-- Book Now Button -->
                    <div class="row pt-5">
                        <div class="col d-flex justify-content-center">
                            <a href="book_1.php" class="btn pink-button font-title d-flex flex-column align-items-center px-5 py-3 shadow">
                                BOOK NOW
                                <img src="images/logo-key-brown.png" alt="key" style="height:18px;">
                            </a>
                        </div>
                    </div>

                </div>

            </div>
        </div>
    </section>


    <!-- SECOND SECTION: Your Best Stay Awaits -->
    <section class="bg-lightpink py-5">
        <div class="container px-4">
        
            <div class="text-center text-black mb-5">
                <h2 class="font-title fw-bold">Your Best Stay Awaits</h2>

                <p class="font-body">
                    Cozy yet elegant rooms, featuring a vintage charm that make every stay feel warm and refined.
                </p>
            </div>

            <div class="row text-center g-4">

                <div class="col-md-4">
                    <div class="bg-lightbrown text-white h-100 shadow rounded-3 overflow-hidden">

                        <img src="images/hotel_pictures/standard1.png"
                            class="img-fluid w-100"
                            alt="" style="height: 250px; object-fit: cover;">

                        <div class="p-4">
                            <h1 class="font-title fw-bold fs-3">Standard Room</h1>

                            <p class="font-body fs-6 my-3">
                                Enjoy comfort and simplicity in our thoughtfully designed Standard Room.
                            </p>

                            <a href="rooms.php#standard-room" class="btn btn-link font-pink fs-5 text-decoration-none">
                            Learn More
                            </a>
                        </div>

                    </div>
                </div>

                <div class="col-md-4">
                    <div class="bg-lightbrown text-white h-100 shadow">

                        <img src="images/hotel_pictures/deluxe1.jpg"
                            class="img-fluid w-100"
                            alt="" style="height: 250px; object-fit: cover;">

                        <div class="p-4">
                            <h1 class="font-title fw-bold fs-3">Deluxe Room</h1>

                            <p class="font-body fs-6 my-3">
                                Upgrade your stay with our Deluxe Room, featuring a more spacious layout and enhanced amenities.
                            </p>

                            <a href="rooms.php#deluxe-room" class="btn btn-link font-pink fs-5 text-decoration-none">
                            Learn More
                            </a>
                        </div>

                    </div>
                </div>

                <div class="col-md-4">
                    <div class="bg-lightbrown text-white h-100 shadow rounded-3 overflow-hidden">

                        <img src="images/hotel_pictures/suite1.jpg"
                            class="img-fluid w-100"
                            alt="" style="height: 250px; object-fit: cover;">

                        <div class="p-4">
                            <h1 class="font-title fw-bold fs-3">Suite Room</h1>

                            <p class="font-body fs-6 my-3">
                                Experience premium luxury in our Suite Room, designed for families or guests seeking the ultimate comfort.
                            </p>

                            <a href="rooms.php#suite-room" class="btn btn-link font-pink fs-5 text-decoration-none">
                            Learn More
                            </a>
                        </div>

                    </div>
                </div>

            </div>
        </div>
    </section>

    <!-- THIRD SECTION: World-Class Fine Dining -->
    <section class="bg-lightbrown py-5">
        <div class="container px-4">

            <div class="text-center text-white mb-5">
                <h2 class="font-title fw-bold">World-Class Fine Dining</h2>

                <p class="font-body">
                    Dining at the Grand Budapest Hotel is elegant yet charming, with delicious pastries, fine dishes, and attentive service.
                </p>
            </div>

            <div class="row text-center g-4">

                <div class="col-md-4">
                    <div class="bg-lightpink text-black h-100 shadow rounded-3 overflow-hidden">
                        <img src="images/hotel_pictures/buffet1.jpg"
                            class="img-fluid w-100"
                            alt="" style="height: 250px; object-fit: cover;">

                        <div class="p-4">
                            <h1 class="font-title fw-bold fs-3">Buffet</h1>

                            <p class="font-body fs-6 my-4">
                                Indulge in a lavish buffet experience featuring a curated selection of international and local cuisine.
                            </p>

                            <a href="amenities_dining.php" class="btn btn-link font-darkbrown fs-5 text-decoration-none">
                            Learn More
                            </a>
                        </div>
                    </div>
                </div>

                <div class="col-md-4">
                    <div class="bg-lightpink text-black h-100 shadow rounded-3 overflow-hidden">
                        <img src="images/hotel_pictures/restaurant2.jpg"
                            class="img-fluid w-100"
                            alt="" style="height: 250px; object-fit: cover;">

                        <div class="p-4">
                            <h1 class="font-title fw-bold fs-3">Restaurant</h1>

                            <p class="font-body fs-6 my-3">
                                Experience fine dining at its finest in our signature restaurant. Our chefs craft each dish with precision, using high-quality ingredients to deliver a perfect balance of flavor and presentation.
                            </p>

                        <a href="amenities_dining.php" class="btn btn-link font-darkbrown fs-5 text-decoration-none">
                            Learn More
                            </a>
                        </div>
                    </div>
                </div>

                <div class="col-md-4">
                    <div class="bg-lightpink text-black h-100 shadow rounded-3 overflow-hidden">
                        <img src="images/hotel_pictures/bar1.jpg"
                            class="img-fluid w-100"
                            alt="" style="height: 250px; object-fit: cover;">

                        <div class="p-4">
                            <h1 class="font-title fw-bold fs-3">Bar</h1>

                            <p class="font-body fs-6 my-3">
                                Unwind and relax at our elegant bar, where timeless charm meets modern taste. Enjoy handcrafted cocktails, premium wines, and a wide selection of spirits in a cozy yet refined atmosphere.
                            </p>

                            <a href="amenities_dining.php" class="btn btn-link font-darkbrown fs-5 text-decoration-none">
                            Learn More
                            </a>
                        </div>
                    </div>
                </div>

            </div>
        </div>
    </section>

    <!-- FOURTH SECTION: Relax and Unwind -->
    <section class="bg-lightpink py-5">
        <div class="container px-4">

            <div class="text-center text-black mb-5">
                <h2 class="font-title fw-bold">Relax and Unwind</h2>

                <p class="font-body">
                    The gym, pool, and spa offer a perfect place to relax and recharge, combining comfort with luxury.
                </p>
            </div>

            <div class="row text-center g-4">

                <div class="col-md-4">
                    <div class="bg-lightbrown text-white h-100 shadow rounded-3 overflow-hidden">
                        <img src="images/hotel_pictures/gym.jpg"
                            class="img-fluid w-100"
                            alt="" style="height: 250px; object-fit: cover;">

                        <div class="p-4">
                            <h1 class="font-title fw-bold fs-3">Gym</h1>

                            <p class="font-body fs-6 my-3">
                            A well-equipped space designed to keep you active and energized during your stay.
                            </p>

                            <a href="amenities_activities.php" class="btn btn-link font-pink fs-5 text-decoration-none">
                            Learn More
                            </a>
                        </div>
                    </div>
                </div>

                <div class="col-md-4">
                    <div class="bg-lightbrown text-white h-100 shadow rounded-3 overflow-hidden">
                        <img src="images/hotel_pictures/spa.png"
                            class="img-fluid w-100"
                            alt="" style="height: 250px; object-fit: cover;">

                        <div class="p-4">
                            <h1 class="font-title fw-bold fs-3">Spa</h1>

                            <p class="font-body fs-6 my-3">
                                A calm, relaxing space offering treatments designed to ease stress and restore balance.
                            </p>

                            <a href="amenities_activities.php" class="btn btn-link font-pink fs-5 text-decoration-none">
                            Learn More
                            </a>
                        </div>
                    </div>
                </div>

                <div class="col-md-4">
                    <div class="bg-lightbrown text-white h-100 shadow rounded-3 overflow-hidden">
                        <img src="images/hotel_pictures/pool.jpg"
                            class="img-fluid w-100"
                            alt="" style="height: 250px; object-fit: cover;">

                        <div class="p-4">
                            <h1 class="font-title fw-bold fs-3">Pool</h1>

                            <p class="font-body fs-6 my-3">
                                A serene swimming area perfect for a refreshing dip or quiet relaxation.
                            </p>

                            <a href="amenities_activities.php" class="btn btn-link font-pink fs-5 text-decoration-none">
                            Learn More
                            </a>
                        </div>
                    </div>
                </div>

            </div>
        </div>
    </section>

    <!-- FIFTH SECTION: Come Visit Us -->
    <section class="bg-lightbrown py-5">
        <div class="container text-center px-4">
            <div class="row g-4 align-items-center"> 
                
                <div class="col-12 col-md-6 text-white">
                    <div>
                        <h2>Come Visit Us</h2>
                    </div>
                    <div class="mt-3">
                        <p>location: 1 Alpine Summit Drive Lutz, Zubrowka 1099 Republic of Zubrowka</p>
                        <p>Contact: +63 975 714 1559</p>
                        <p>Email: reservations@grandbudapest.zb</p>
                    </div>
                    <div class="d-flex justify-content-center align-items-center gap-5 py-3">
                        <a href="https://www.facebook.com/"><img src="images/logo-fb-white.png" alt="" style="width:50px;"></a>
                        <a href="https://www.instagram.com/"><img src="images/logo-ig-white.png" alt="" style="width: 50px;"></a>
                        <a href="https://www.tiktok.com/"><img src="images/logo-tiktok-white.png" alt="" style="width: 50px;"></a>
                    </div>
                    
                    <div class="py-4">
                        <div class="col d-flex justify-content-center">
                            <a href="contact.php" class="btn pink-button font-title d-flex flex-column align-items-center px-5 py-3 shadow">
                                CONTACT US
                            </a>
                        </div>
                    </div>
                </div>

                <div class="col-12 col-md-6 px-4">
                    <iframe 
                        src="https://www.google.com/maps/embed?pb=!1m18..."
                        class="rounded shadow map-frame w-100"
                        style="border:0; min-height: 300px;"
                        allowfullscreen=""
                        loading="lazy">
                    </iframe>
                </div>

            </div>
        </div>
    </section>

    <!-- FOOTER -->
    <?php include 'footer.php'; ?>

    <script src="js/bootstrap.bundle.min.js"></script>


</body>
</html>